  ...
  const getMealTypesTotal = () => {
    ...
    food_data.forEach((item, index) => {
      if (mealTypes.breakfast === item.meal_type.toLowerCase()) {
        results.breakfast += Math.round(item.nf_calories);
      } else if (mealTypes.snack === item.meal_type.toLowerCase()) {
        results.snack += Math.round(item.nf_calories);
      } else if (mealTypes.lunch === item.meal_type.toLowerCase()) {
        results.lunch += Math.round(item.nf_calories);
      } else if (mealTypes.dinner === item.meal_type.toLowerCase()) {
        results.dinner += Math.round(item.nf_calories);
      }
    });
    return results;
  };
        ...
        <li className="diet-details-container">
          <div className="diet-goal-container">
            <div className="diet-consumed-container">
              ...
              <div style={{ color: "#BEBEBE" }}>consumed</div>
            </div>
          ...
          <div className="diet-types-container">
            <div className="diet-type">
              ...
              <div style={{ color: "#808080" }}>Breakfast</div>
            </div>
            <div className="diet-type">
              <h6>{getMealTypesTotal().lunch} </h6>
              <div style={{ color: "#808080" }}>Lunch</div>
            </div>
            <div className="diet-type">
              <h6>{getMealTypesTotal().dinner} </h6>
              <div style={{ color: "#808080" }}>Dinner</div>
            </div>
            <div className="diet-type">
              <h6>{getMealTypesTotal().snack} </h6>
              <div style={{ color: "#808080" }}>Snack</div>
            </div>
          </div>
        </li>
        {displayFoodItems()}
