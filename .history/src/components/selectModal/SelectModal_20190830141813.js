...
const SelectModal = () => {
  ...
  useEffect(() => {
    if (clickedItemId) {
      ...
      const foodTypes = clickedItemId.split("-");
      const key = foodTypes[1] ? "branded" : "common";
      foodContext.showDetails(key);
    }
  });
  ...
  const getCommonItems = () => {
    ...
    if (food_items_length > 0) {
      items = foods.common.map((item, index) => {
        const brandId = item.nix_brand_id ? item.nix_brand_id : null;
        if (index <= 4) {
          return (
            <>
              <li
