...
const DetailsModal = () => {
  ...
  const [servingsSize, setServingsSize] = useState(2.0);
  ...
  useEffect(() => {
    if (foodItemClicked) {
      if (nutrientInfo) {
        ...
        setFoodGrams(
          Math.round(nutrientInfo.foods[0].serving_weight_grams) * servingsSize
        );
        setFoodCalories(
          Math.round(nutrientInfo.foods[0].nf_calories) * servingsSize
        );
